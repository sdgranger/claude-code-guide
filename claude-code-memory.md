...
### 2.4 CLAUDE.local.md (개인용 비공개 지침)

```
프로젝트/
├── CLAUDE.md          # 팀 공유 (git 체크인)
└── CLAUDE.local.md    # 개인 (gitignore에 추가)
```

> CLAUDE.md 옆에 함께 로드되지만 git에서 제외됨. `/init`에서 personal 옵션 선택 시 자동으로 .gitignore 처리.
> 같은 디렉토리에 CLAUDE.md와 CLAUDE.local.md 둘 다 있으면 **local.md가 뒤에 추가**되어 충돌 시 개인 지침이 우선.

### 2.5 HTML 주석으로 사람용 메모

```markdown
<!-- 이 메모는 Claude 컨텍스트에 들어가지 않음 -->
<!-- 유지보수자만 보는 노트, TODO, 변경 이력 등 -->

# 실제 지침
- 2칸 들여쓰기 사용
```

> **블록 레벨 HTML 주석은 Claude 컨텍스트 주입 전에 제거**됩니다.
> 사람만 보면 되는 노트는 여기 적어 토큰 낭비를 피하세요.
> 코드 블록 내부의 주석은 보존됩니다 (Read tool로 직접 열어도 보임).

### 2.6 AGENTS.md와의 관계

다른 AI 도구가 `AGENTS.md`를 사용하더라도, **Claude는 `CLAUDE.md`만** 읽습니다.
공유하려면 CLAUDE.md에서 AGENTS.md를 import:
...
- [ ] CLAUDE.local.md로 개인 지침 분리해보기
- [ ] HTML 주석으로 사람용 메모 작성해보기
...
Claude가 자동으로 분류하여 저장하는 메모리 타입:

| 타입 | 내용 | 예시 |
|------|------|------|
| **user** | 사용자의 역할, 목표, 책임, 지식 수준 | "사용자는 데이터 사이언티스트, 현재 로깅 관찰성 작업 중" |
| **feedback** | 사용자가 준 작업 방식 가이드 (피해야 할 것/계속할 것) | "통합 테스트는 mock DB 금지, 실제 DB 사용. 이유: 마이그레이션 사고 경험" |
| **project** | 진행 중인 작업, 목표, 사건의 맥락 | "2026-03-05 머지 동결 시작 (모바일 릴리스 컷)" |
| **reference** | 외부 시스템 위치 포인터 | "파이프라인 버그는 Linear 'INGEST' 프로젝트에서 추적" |
